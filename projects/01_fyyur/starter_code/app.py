...
def show_venue(venue_id):
  ...
  pastShowsJoin = db.session.query(Show.id, Show.artist_id, Show.start_time, Artist.name, Artist.image_link).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time < datetime.now()).all()
  upcomingShowsJoin = db.session.query(Show.id, Show.artist_id, Show.start_time, Artist.name, Artist.image_link).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time > datetime.now()).all()

  pastShowList = []
  for pastShow in pastShowsJoin:
        artistEntry = {
  ...
  upcomingShowList = []
  for upcomingShow in upcomingShowsJoin:
        artistEntry = {
  ...
  dataNew = {
    ...
    "past_shows_count": len(pastShowsJoin),
    "upcoming_shows_count": len(upcomingShowsJoin),
  }
...
def show_artist(artist_id):
  ...
  pastShowsJoin = db.session.query(Show.id, Show.venue_id, Show.start_time, Venue.name, Venue.image_link).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time < datetime.now()).all()
  upcomingShowsJoin = db.session.query(Show.id, Show.venue_id, Show.start_time, Venue.name, Venue.image_link).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time > datetime.now()).all()
...
def shows():
  ...
  showsJoin = db.session.query(Show.venue_id, Show.artist_id, Show.start_time, Venue.name.label("venue_name"), Artist.name.label("artist_name"), Artist.image_link).join(Venue).join(Artist).all()
  showList = []
  for show in showsJoin:
        showEntry = {
